...
$(document).ready(
  ...
  $.getJSON(jsonFile).done(function (data) {
    //Usues the getJSON plugin from JQuery to collect data from the jsonFile, and once it has done that
    //Calls and function using that data.
    var model = [];
    //Creates a blank array to be used in the following for each.
    $.each(data.koTeams, function (i, item) {
      model.push(item.teamName);
    });
    //Runs a for each loop using the JQuery library, using the data taken from the koTeams object in the JSON
    //For each iteration, it pushes the teamName value into the array.
    ko.applyBindings(new viewModel(model));
    //Initialises the data-binds and creates the viewModel using the data that is held within the model variable.
  });
